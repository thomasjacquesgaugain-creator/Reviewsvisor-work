...
export default function SavedEstablishmentsList({
  ...
  const activePlaceId = useEstablishmentStore(
    (s) => s.activePlaceId ?? s.selectedEstablishment?.place_id ?? null,
  );
  const setActivePlace = useEstablishmentStore((s) => s.setActivePlace);
  const [checkingSubscription, setCheckingSubscription] = useState(false);
  const [showSubscriptionModal, setShowSubscriptionModal] = useState(false);
  const [creatingCheckout, setCreatingCheckout] = useState(false);
  const [modalEstablishmentCount, setModalEstablishmentCount] = useState<
    number | null
  >(null);
  ...
  const activePlan = useMemo(() => {
    if (!subscription.subscribed || !subscription.price_id) {
      ...
      return (
        subscriptionPlans.find((p) => p.id === "pro-engagement") ||
        subscriptionPlans[0]
      );
    }
  ...
  const pricingInfo = useMemo(() => {
    ...
    return {
      ...
      formattedAddonTotal:
        addonTotal > 0 ? `+${addonTotal.toFixed(2).replace(".", ",")} €` : null,
    };
  ...
  const currentEstablishmentsCount =
    modalEstablishmentCount !== null
      ? modalEstablishmentCount
  ...
  const setModalCountFromEstablishmentsTable = useCallback(async () => {
    ...
    if (error) {
      console.error(
        "[SavedEstablishmentsList] Erreur comptage établissements:",
        error,
      );
      return;
    }
  ...
  const loadEstablishmentsFromDb = useCallback(async (): Promise<number> => {
    try {
      ...
      console.log(
        "Establishments from DB (raw query, user_id=" + user.id + "):",
        etablissements,
      );
      console.log(
        "Establishments from DB (count):",
        (etablissements || []).length,
      );
      if (etablissements?.length) {
        etablissements.forEach(
          (e: { id?: string; place_id?: string; nom?: string }, i: number) => {
            console.log(
              `  [${i}] id=${e.id}, place_id=${e.place_id}, nom=${e.nom}`,
            );
          },
        );
      }
      ...
      // To fetch current_establishment
      ...
      const activeEtab = current_establishment;
      ...
      console.log(
        "Establishments from DB (mapped, setEstablishments):",
        sortedDbList,
      );
      ...
      if (activeEtab) {
        ...
        setActivePlace(firstEtab.place_id, {
          ...
          window.dispatchEvent(
            new CustomEvent(EVT_SAVED, { detail: firstEtab }),
          );
        });
      }
  ...
  const handleAddClick = async () => {
    ...
    try {
      ...
      if (!import.meta.env.PROD) {
        ...
        console.log(
          "[SavedEstablishmentsList] Current establishments:",
          establishments.length,
        );
      }
      ...
      if (!status.subscribed) {
        if (!import.meta.env.PROD) {
          console.log(
            "[SavedEstablishmentsList] User NOT subscribed, showing subscription modal",
          );
        }
      ...
      if (establishments.length >= INCLUDED_ESTABLISHMENTS) {
        if (!import.meta.env.PROD) {
          console.log(
            "[SavedEstablishmentsList] Quota exceeded, showing addon modal",
            {
      ...
      if (!import.meta.env.PROD) {
        console.log(
          "[SavedEstablishmentsList] Under quota, allowing navigation",
        );
      }
      onAddClick?.();
    } catch (error) {
      console.error(
        "[SavedEstablishmentsList] Error checking subscription:",
        error,
      );
  ...
  const handleProceedToCheckout = async () => {
    if (!import.meta.env.PROD) {
      console.log(
        "[SavedEstablishmentsList] handleProceedToCheckout triggered",
      );
    }
    ...
    try {
      ...
      if (isCreator()) {
        ...
        if (!subscription.subscribed) {
          if (!import.meta.env.PROD) {
            console.log(
              "[SavedEstablishmentsList] Creator: Activating pro plan first",
            );
          }
          const proResult = await activateCreatorSubscription(
            PRODUCT_KEYS.PRO_1499_12M,
          );
          if (!proResult.success) {
            sonnerToast.error(
              proResult.error || t("subscription.proActivationError"),
            );
            return;
          }
        }
      ...
      if (data?.url) {
        if (!import.meta.env.PROD) {
          console.log(
            "[SavedEstablishmentsList] Redirecting to Stripe checkout:",
            data.url,
          );
        }
  ...
  const handleSelectEstablishment = async (etab: Etab) => {
    ...
    try {
      ...
      document
        .querySelector('[data-testid="card-mon-etablissement"]')
        ?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
        ...
        <div className="flex flex-wrap gap-3">
          {establishments.map((etab) => (
            <EstablishmentItem
              ...
              isActive={activePlaceId === etab.place_id}
            />
          ))}
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {activePlan.id === "pro-engagement"
                      ? t("subscription.commitment12Months")
                      : t("subscription.withoutCommitment")}
                  </p>
                </div>
                ...
                <div className="flex items-start justify-between gap-2 mb-0.5">
                  <div>
                    <h4 className="text-base font-bold text-foreground">
                      ...
                      {modalPricingInfo.additionalCount > 0 && (
                    ...
                    <p className="text-xs text-muted-foreground">
                      {modalPricingInfo.additionalCount === 0
                        ? t("subscription.firstEstablishmentIncluded")
                        : t("subscription.additionalEstablishmentPrice", {
                            count: modalPricingInfo.additionalCount,
                            price: "4,99",
                          })}
                    </p>
                  </div>
                ...
                <ul className="space-y-0">
                  <li className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-foreground leading-tight">
                      {t("subscription.centralizeAllLocations")}
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="inline-flex w-4 h-4 rounded-full bg-purple-100 items-center justify-center flex-shrink-0">
                    ...
                    <span className="text-sm text-foreground leading-tight">
                      {t("subscription.idealForGroupsFranchises")}
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="inline-flex w-4 h-4 rounded-full bg-purple-100 items-center justify-center flex-shrink-0">
                    ...
                    <span className="text-sm text-foreground leading-tight">
                      {t("subscription.independentTrackingPerEstablishment")}
                    </span>
                  </li>
                </ul>
              </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {modalPricingInfo.newCount === 1
                      ...
                      : t("subscription.establishmentsAfterAdd", {
                          count: modalPricingInfo.newCount,
                        })}
                  </p>
                </div>
