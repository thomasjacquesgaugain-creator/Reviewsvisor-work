  ...
  return <div className="relative" ref={boxRef}>
      <div className="relative">
        <div className="pointer-events-none absolute inset-y-0 left-3 flex items-center">
          {loading ? <Loader2 className="h-5 w-5 text-muted-foreground animate-spin" /> : <Search className="h-5 w-5 text-muted-foreground" />}
        </div>
        
      </div>

      {items.length > 0 && <div className="absolute left-0 right-0 top-full z-20 mt-1 rounded-2xl border bg-white shadow-lg overflow-hidden">
          {items.map((it, i) => <button key={it.id} type="button" onClick={() => select(it)} onMouseEnter={() => setHi(i)} className={`flex w-full items-start px-3 py-2 text-left hover:bg-gray-50 ${i === hi ? "bg-gray-100" : ""}`}>
