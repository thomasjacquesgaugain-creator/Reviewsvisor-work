...
export async function runAnalyze(body: Payload) {
  // A) Proxy serveur (même origine) — évite tout CORS/iframe
  try {
    const rp = await fetch('/api/analyze', {
      method: 'POST',
    ...
    if (!rp.ok) throw new Error(js?.error || txt || `HTTP ${rp.status}`);
    if (js?.error) throw new Error(String(js.error));
    return js;
  } catch (_) {
    // B) supabase.functions.invoke
    try {
      ...
      // C) Fetch brut via URL/headers internes du client
