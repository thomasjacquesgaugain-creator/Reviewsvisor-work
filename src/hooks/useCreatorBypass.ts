...
export function useCreatorBypass() {
  ...
  const activateCreatorSubscription = useCallback(
    async (
      ...
      console.log(`[useCreatorBypass] Starting call to ${functionName}`, {
        productKey,
        url: functionUrl,
      });
      ...
      try {
        ...
        console.log(
          "[useCreatorBypass] Session valid, invoking edge function...",
        );
        ...
        console.log("[useCreatorBypass] Edge function response:", {
          data,
          error,
        });
        ...
        if (error) {
          ...
          const errorDetails =
            error.context?.message || error.message || "Erreur inconnue";
