...
const Dashboard = () => {
  ...
  const [selectedNoteCategory, setSelectedNoteCategory] = useState<
    string | null
  >(null);
  const [selectedPriorityCategory, setSelectedPriorityCategory] = useState<
    string | null
  >(null);
  ...
  const [establishmentDbId, setEstablishmentDbId] = useState<string | null>(
    null,
  );
  ...
  const getInitialSnapshot = () => {
    const pid =
      useEstablishmentStore.getState().selectedEstablishment?.place_id ?? null;
    return pid ? getDashboardSnapshot(pid) : null;
  };
  const [insight, setInsight] = useState<any>(
    () => getInitialSnapshot()?.insight ?? null,
  );
  ...
  >({});
  const [validatedReviews, setValidatedReviews] = useState<Set<number>>(
    new Set(),
  );
  const [validatedResponsesText, setValidatedResponsesText] = useState<
    Map<number, string>
  >(new Map());
  const [isValidatingReview, setIsValidatingReview] = useState<
    Record<number, boolean>
  >({});
  const [pendingReviews, setPendingReviews] = useState<any[]>([]);
  const [reponsesStats, setReponsesStats] = useState<{
    validated: number;
    total: number;
  }>({ validated: 0, total: 0 });
  ...
  const getValidatedCountFromLocalStorage = (placeId?: string): number => {
    ...
    try {
      ...
      console.error(
        "Error reading validated reviews from localStorage:",
        error,
      );
      return 0;
    }
  ...
  const [actionPlanChecks, setActionPlanChecks] = useState<
    Record<string, boolean>
  >({});
  ...
  const [progressBaselinePlaceId, setProgressBaselinePlaceId] = useState<
    string | null
  >(null);
  const [progressBaselineRating, setProgressBaselineRating] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (progressBaselinePlaceId !== placeId) {
      ...
      setProgressBaselineRating(
        hasReviews && currentRating > 0 ? currentRating : null,
      );
      return;
    }
  ...
  const progressionStats = useMemo(() => {
    ...
    const checklistRatio =
      checklistTotal > 0 ? clamp01(checklistChecked / checklistTotal) : 0;
    ...
    const planChecked = Object.entries(actionPlanChecks).reduce(
      (acc, [k, v]) => {
        if (!v) return acc;
  ...
  const establishmentContext = useMemo(() => {
    ...
    sample.forEach((r: any) => {
      ...
      if (!txt) return;
      (Object.keys(sectorSignals) as Array<keyof typeof sectorSignals>).forEach(
        (k) => {
          if (sectorSignals[k].some((s) => txt.includes(normalize(s))))
            scores[k] += 1;
        },
      );
    });
  ...
  const generateActionPlan = (
    ...
    if (
      !insightData?.top_issues ||
      !Array.isArray(insightData.top_issues) ||
      insightData.top_issues.length === 0
    ) {
      console.log(
        "[generateActionPlan] Aucun problème identifié dans les insights",
      );
      return [];
    }
    ...
    console.log(
      "[generateActionPlan] Problèmes triés:",
      sortedIssues.map((i: any) => ({ theme: i.theme, count: i.count })),
    );
    ...
    sortedIssues.forEach((issue: any, index: number) => {
      ...
      const severity =
        issue.severity || (index < 2 ? "high" : index < 4 ? "medium" : "low");

      console.log(
        `[generateActionPlan] Traitement du problème: ${theme}, count: ${count}, severity: ${severity}`,
      );
      ...
      const actionsToAdd = generatedActions.slice(
        0,
        priority === "high" ? 2 : 1,
      );
    ...
    console.log(
      "[generateActionPlan] Actions générées:",
      actions.length,
      actions.map((a) => ({ title: a.title, priority: a.priority })),
    );
  ...
  const generateConsultantAdvice = (
    insightData: any,
    reviewsData: any[],
    validatedReviewsSet: Set<number>,
  ): string[] => {
    ...
    const softenAdvice = (text: string): string => {
      ...
      t = t.replace(
        /C'est une tendance à surveiller\b/gi,
        "Pourquoi ne pas garder un œil bienveillant sur ce point",
      );
      t = t.replace(
        /C'est un point récurrent à analyser\b/gi,
        "Tu pourrais essayer de regarder ce point de plus près",
      );
      t = t.replace(
        /C'est un point (secondaire )?à analyser\b/gi,
        "Tu pourrais essayer d’explorer ce sujet tranquillement",
      );
      t = t.replace(
        /C'est un point secondaire à surveiller\b/gi,
        "Pourquoi ne pas garder ce point en tête",
      );
    ...
    if (topIssues.length > 0) {
      ...
      const issuePercentage =
        totalReviews > 0 ? Math.round((issueCount / totalReviews) * 100) : 0;
    ...
    if (topStrengths.length > 0) {
      ...
      const strengthPercentage =
        totalReviews > 0 ? Math.round((strengthCount / totalReviews) * 100) : 0;

      if (strengthPercentage >= 40) {
        if (
          strengthTheme.includes("ambiance") ||
    ...
    if (topIssues.length >= 2) {
      ...
      const secondIssuePercentage =
        totalReviews > 0
          ? Math.round((secondIssueCount / totalReviews) * 100)
          : 0;

      if (secondIssuePercentage >= 20) {
        advice.push(
          `En plus du problème principal, ${translateTheme(secondIssueTheme)} revient aussi souvent (${secondIssuePercentage}% des avis). C'est un point secondaire à analyser.`,
        );
      }
    }
    ...
    if (topStrengths.length > 0 && topStrengths.length >= 2) {
      ...
      const secondStrengthTheme = (
        secondStrength.theme ||
        secondStrength.strength ||
        secondStrength
      ).toLowerCase();
      const secondStrengthCount = secondStrength.count || 0;
      const secondStrengthPercentage =
        totalReviews > 0
          ? Math.round((secondStrengthCount / totalReviews) * 100)
          : 0;

      if (secondStrengthPercentage >= 25) {
        advice.push(
          `${translateTheme(secondStrengthTheme)} revient souvent dans les avis positifs (${secondStrengthPercentage}%). C'est un point fort à prendre en compte.`,
        );
      }
    }
    ...
    if (positiveReviews.length > 0 && avgRating >= 4.0) {
      const recentPositiveReviews = positiveReviews.filter((r: any) => {
        ...
        const daysAgo =
          (new Date().getTime() - reviewDate.getTime()) / (1000 * 60 * 60 * 24);
        return daysAgo <= 30;
      });

      if (recentPositiveReviews.length >= 5) {
        advice.push(
          `Tu as ${recentPositiveReviews.length} avis positifs récents. C'est une tendance positive dans les retours clients.`,
        );
      }
    }
    ...
    if (topIssues.length >= 3) {
      ...
      const thirdIssuePercentage =
        totalReviews > 0
          ? Math.round((thirdIssueCount / totalReviews) * 100)
          : 0;

      if (thirdIssuePercentage >= 15) {
        advice.push(
          `${translateTheme(thirdIssueTheme)} revient aussi (${thirdIssuePercentage}% des avis). C'est un point secondaire à surveiller.`,
        );
      }
    }
    ...
    if (topStrengths.length > 0 && avgRating >= 4.0) {
      ...
      if (
        ...
        advice.push(
          `L'ambiance est un point fort récurrent selon tes clients. C'est un atout à conserver.`,
        );
      } else if (
        strengthTheme.includes("qualité") ||
        strengthTheme.includes("goût")
        ...
        advice.push(
          `Ta cuisine est souvent appréciée dans les avis. C'est un point fort à valoriser.`,
        );
      }
    }
    ...
    if (
      ...
      negativeReviews.length < positiveReviews.length
    ) {
      ...
      if (recentNegativeReviews.length > 0) {
        advice.push(
          `Tu as ${recentNegativeReviews.length} avis négatifs récents. C'est une tendance à surveiller dans tes retours clients.`,
        );
      }
    }
    ...
    if (totalReviews >= 20) {
      ...
      const veryPositivePercentage = Math.round(
        (veryPositiveCount / totalReviews) * 100,
      );

      if (veryPositivePercentage >= 30) {
        advice.push(
          `${veryPositivePercentage}% de tes clients donnent 5 étoiles. C'est une tendance très positive dans les retours.`,
        );
      }
    }
  ...
  const generateChecklistActions = (
    insightData: any,
    reviewsData: any[],
    validatedReviewsSet: Set<number>,
  ): ChecklistAction[] => {
    ...
    const negativeReviews = reviewsData.filter(
      (r: any) => (r.rating || 0) <= 2 && !validatedReviewsSet.has(r.id),
    );
    const positiveReviews = reviewsData.filter(
      (r: any) => (r.rating || 0) >= 5,
    );
    ...
    if (topIssues.length > 0) {
      ...
      const isValidIssue =
        issueTheme && issueTheme.trim().length > 0 && issueCount > 0;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedActions) {
        ...
        const newActions = generateChecklistActions(
          insight,
          allReviewsForChart,
          validatedReviews,
        );
  ...
  const handleChecklistActionComplete = (actionId: string) => {
    setChecklistActions((prev) => {
      ...
      const allCompleted =
        updated.length === 5 && updated.every((a) => a.completed);
  ...
  const loadEstablishmentsFromDB = async () => {
    ...
    try {
      ...
      if (mapped.length > 0) {
        const placeId =
          useEstablishmentStore.getState().activePlaceId ??
          useEstablishmentStore.getState().selectedEstablishment?.place_id ??
          null;
        const active = placeId
          ? mapped.find((e) => e.place_id === placeId)
          : null;
  ...
  const kpiMemoryCacheRef = useRef<
    Record<string, { insight: any; reviews: any[] }>
  >({});
  ...
  useLayoutEffect(() => {
    ...
    if (
      fromMemory?.insight ||
      (fromMemory?.reviews && fromMemory.reviews.length > 0)
    ) {
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      ...
      let placeId: string | null =
        selectedEtab?.place_id ?? selectedEstablishment?.place_id ?? null;
      let currentEstabName =
        selectedEtab?.name ?? selectedEstablishment?.name ?? null;
      ...
      if (!placeId) {
        ...
        if (activeRow?.place_id) {
          ...
          currentEstabName = activeRow.name;
        }
      }
      ...
      try {
        ...
        if (result.success) {
          if (result.hasAnalysis && result.data) {
            ...
            if (!import.meta.env.PROD) {
              console.log(
                "[Dashboard] ✅ Analyse chargée automatiquement:",
                result.data,
              );
            }
          } else {
            ...
            if (!import.meta.env.PROD) {
              console.log(
                "[Dashboard] ℹ️ Aucune analyse pour place_id:",
                placeId,
              );
            }
          }
        ...
        if (reviewsData && reviewsData.length >= 0) {
          ...
          if (!reviewsData || reviewsData.length === 0) {
            console.warn(
              "[PROTECTION DONNÉES] ⚠️ ATTENTION: Aucun avis trouvé pour cet établissement - vérifier la source de données",
              {
                place_id: currentEstab.place_id,
                user_id: user.id,
              },
        ...
        setByRating(counts);
            ...
            const statsByPlatform: Record<
              string,
              { count: number; totalRating: number; avgRating: number }
            > = {};
            ...
            Object.keys(statsByPlatform).forEach((source) => {
              ...
              stat.avgRating =
                stat.count > 0 ? stat.totalRating / stat.count : 0;
            });
            ...
            if (currentEstab?.place_id && user?.id) {
              ...
              if (responsesData) {
                ...
                const pending = reviewsData.filter((review) => {
                  ...
                  const hasOwnerReply = !!(
                    review.owner_reply_text && review.owner_reply_text.trim()
                  );
                  const hasRespondedAt = !!review.responded_at;
                  return (
                    !hasValidatedResponse && !hasOwnerReply && !hasRespondedAt
                  );
                });
                ...
                if (currentEstab?.place_id && user?.id) {
                  const stats = await getReponsesStats(
        ...
        if (!estError && establishmentData) {
          ...
          if (!import.meta.env.PROD) {
            console.log(
              "[Dashboard] ✅ Établissement ID récupéré:",
              establishmentData.id,
            );
          }
        } else {
          console.warn(
            "[Dashboard] ⚠️ Établissement non trouvé dans la base pour:",
            {
              place_id: currentEstab.place_id,
              user_id: user.id,
              error: estError,
          ...
          if (reviewsData && reviewsData.length > 0) {
            ...
            if (oldestReview) {
              setEstablishmentCreatedAt(
                oldestReview.published_at || oldestReview.inserted_at,
              );
            }
          }
        }
  ...
  const displayTotalAnalyzed: number | null = showKpiNumbers
    ? totalAnalyzed
    : null;
  const displayAvgRating: number | null = showKpiNumbers ? avgRating : null;
  const displayTotalReviews: number | null = showKpiNumbers
    ? totalReviews
    : null;
  ...
  const reviewsForAnalysis: Review[] = useMemo(
    () =>
  ...
  const analysisDataForTab: CompleteAnalysisData | null = useMemo(() => {
    try {
      ...
      if (reviewsForAnalysis.length > 0) {
        ...
        const avg =
          reviewsForAnalysis.reduce((s, r) => s + (r.note || 0), 0) / n;
        const positiveCount = reviewsForAnalysis.filter(
          (r) => (r.note || 0) >= 4,
        ).length;
  ...
  const paretoData =
    topIssues.length > 0
      ? topIssues.slice(0, 3).map((issue: any, index: number) => {
          ...
          const percentage =
            totalAnalyzed > 0 ? (count / totalAnalyzed) * 100 : 0;
          return {
            name: translateTheme(
              issue.theme ||
                issue.issue ||
      ...
      ? topStrengths.slice(0, 3).map((strength: any, index: number) => {
          ...
          return {
            name: translateTheme(
              strength.theme ||
                strength.strength ||
                t("dashboard.strengthBadge", { number: index + 1 }),
            ),
            count,
            percentage,
            cumulative: 0, // Will be calculated below
  ...
  const getReviewDisplayDate = (
    review: any,
  ): { dateStr: string | null; isImportDate: boolean } => {
    const dateStr =
      review?.published_at ||
      review?.create_time ||
  ...
  const courbeNoteData = useMemo(() => {
    ...
    return getRatingEvolution(
      allReviewsForChart,
      establishmentCreatedAt,
      granularityEvolution,
      i18n.language,
    );
  }, [
    allReviewsForChart,
          ...
          <div className="container mx-auto px-4 py-8">
                  ...
                  <div className="flex items-center gap-2 text-gray-600 mt-1">
                    ...
                    <span>
                      {displayTotalAnalyzed == null
                        ? "—"
                        : t("dashboard.analysisOfReviews", {
                            count: displayTotalAnalyzed,
                          })}
                    </span>
                  </div>
                </div>
                      ...
                      <div className="relative">
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            ...
                            <Popover
                              open={showEstablishmentsDropdown}
                              onOpenChange={setShowEstablishmentsDropdown}
                            >
                    ...
                    onClick={() => {
                      ...
                      if (!currentEstab) {
                        toast.error(t("common.error"), {
                          description: t(
                            "establishment.noEstablishmentSelected",
                          ),
                        });
                        return;
                      }
                      ...
                      if (!hasReviews || allReviewsForChart.length === 0) {
                        toast.info(t("dashboard.noReportAvailable"), {
                          description: t(
                            "dashboard.importReviewsToGenerateReport",
                          ),
                        });
                        return;
                      }
                      ...
                      try {
                        ...
                        if (!import.meta.env.PROD) {
                          console.log(
                            "[Dashboard] 📄 Génération du PDF avec:",
                            reportData,
                          );
                        }
                        ...
                        console.error(
                          "[Dashboard] ❌ Erreur génération PDF:",
                          error,
                        );
                        toast.error(t("common.error"), {
              ...
              <>
                ...
                <div className="grid md:grid-cols-4 gap-6 mb-8">
                  <Card
                    ...
                    onClick={() =>
                      setOpenCard(
                        openCard === "courbeNote" ? null : "courbeNote",
                      )
                    }
                  >
                    <CardContent className="p-6 text-center">
                      <div className="flex items-center justify-center gap-2 mb-2">
                        ...
                        {displayAvgRating == null ? (
                          ...
                          <span className="text-2xl font-bold animate-in fade-in duration-300">
                      ...
                      <Button
                        ...
                        className="absolute bottom-2 right-2 h-6 w-6 p-0 hover:bg-yellow-50"
                      >
                        {openCard === "courbeNote" ? (
                  ...
                  <Card
                    ...
                    onClick={() =>
                      setOpenCard(
                        openCard === "plateformes" ? null : "plateformes",
                      )
                    }
                  >
                    <CardContent className="p-6 text-center">
                      <div className="flex items-center justify-center gap-1 mb-2">
                        {displayTotalReviews == null ? (
                          ...
                          <>
                            <span className="text-2xl font-bold text-blue-600 animate-in fade-in duration-300">
                              {displayTotalReviews}
                            </span>
                  ...
                  <Card
                    ...
                    onClick={() =>
                      setOpenCard(
                        openCard === "avisPositifs" ? null : "avisPositifs",
                      )
                    }
                  >
                    <CardContent className="p-6 text-center">
                      <div className="flex items-center justify-center gap-1 mb-2">
                        {displayPositivePct == null ? (
                          ...
                          <span className="text-2xl font-bold text-green-600 animate-in fade-in duration-300">
                            {displayPositivePct}%
                          </span>
                        )}
                      </div>
                      <p className="text-sm text-gray-600">
                        {t("dashboard.positiveReviews")}
                      </p>
                      <p className="text-xs text-gray-500">
                        {t("dashboard.rating4StarsOrMore")}
                      </p>
                      <Button
                        variant="ghost"
                        size="sm"
                  ...
                  <Card
                    ...
                    onClick={() =>
                      setOpenCard(
                        openCard === "avisNegatifs" ? null : "avisNegatifs",
                      )
                    }
                  >
                    <CardContent className="p-6">
                      ...
                      <Button
                        ...
                        className="absolute bottom-2 right-2 h-6 w-6 p-0 hover:bg-red-50"
                      >
                        {openCard === "avisNegatifs" ? (
                  ...
                  <Card className="mb-8">
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Select
                          value={granularityEvolution}
                          onValueChange={(value) =>
                            setGranularityEvolution(value as Granularity)
                          }
                        >
                          ...
                          {courbeNoteData.length < 2 ? (
                            <div className="text-center py-8">
                              <p className="text-sm text-gray-500">
                                {t("dashboard.limitedDataForGranularity")}
                              </p>
                              <p className="text-xs text-gray-400 mt-1">
                                {t("dashboard.selectWiderPeriodOrWait")}
                              </p>
                            </div>
                          ) : (
                            <>
                      ...
                      <div className="space-y-4">
                        {worstReviews.length > 0 ? (
                              ...
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-medium">
                                    {avis.author || t("dashboard.anonymous")}
                                  </span>
                                  <span className="text-yellow-500">
                                    {"★".repeat(Math.round(avis.rating || 0))}
                                    {"☆".repeat(
                                      5 - Math.round(avis.rating || 0),
                                    )}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs text-gray-500">
                                    {formatReviewDate(avis.published_at)}
                                  </span>
                                </div>
                              </div>
                              <p className="text-sm text-gray-700 italic">
                                "
                                {extractOriginalText(avis.text) ||
                          ...
                          <p className="text-sm text-gray-500 text-center py-4">
                            {t("dashboard.noNegativeReviewsFound")}
                          </p>
                        )}
                      </div>
                    </CardContent>
                ...
                {openCard === "avisPositifs" && (
                  <Card className="mb-8">
                    <CardHeader>
                      ...
                      <p className="text-sm text-gray-600">
                        {t("dashboard.bestReviewsDescription")}
                      </p>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        {topReviews.length > 0 ? (
                          ...
                          <p className="text-sm text-gray-500 text-center py-4">
                            {t("dashboard.noPositiveReviewsFound")}
                          </p>
                        )}
                      </div>
                    </CardContent>
                ...
                {openCard === "plateformes" && (
                  <Card className="mb-8">
                    <CardHeader>
                      ...
                      <p className="text-sm text-gray-600">
                        {t("dashboard.managePlatformPresences")}
                      </p>
                    </CardHeader>
                    <CardContent>
                        ...
                        {Object.entries(platformStats).length > 0 ? (
                          Object.entries(platformStats).map(
                            ([source, stats]) => {
                              ...
                              > = {
                                google: {
                                  name: t("platforms.google"),
                                  color: "bg-blue-50 text-blue-600",
                                  initial: "G",
                                },
                                yelp: {
                                  name: t("platforms.yelp"),
                                  color: "bg-yellow-100 text-yellow-600",
                                  initial: "Y",
                                },
                                tripadvisor: {
                                  name: t("platforms.tripadvisor"),
                          ...
                          <div className="text-center py-4 text-gray-500">
                            <p className="text-sm">
                              {t("dashboard.noPlatformConnected")}
                            </p>
                            <p className="text-xs mt-1">
                              {t(
                                "dashboard.analyzeEstablishmentToSeePlatforms",
                              )}
                            </p>
                          </div>
                        )}
                      ...
                      <div className="space-y-4">
                        ...
                        {insight?.analysis_version === "v2-auto-universal" ? (
                          ...
                          (() => {
                            ...
                            const themesWithPercentages = insight.themes.map(
                              (theme: any) => {
            ...
            {activeTab === "analyse" && (
              <AnalysisTabContent
                ...
                establishmentName={
                  selectedEtab?.name || selectedEstablishment?.name
                }
                isLoading={isLoadingInsight && !analysisDataForTab && !insight}
                isFallbackAnalysis={
                  !!analysisDataForTab &&
                  !insight &&
                  reviewsForAnalysis.length > 0
                }
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                          ...
                          <CardTitle className="text-lg">
                            {t("dashboard.top3PriorityProblems")}
                          </CardTitle>
                        </div>
                        <ChevronDown
                      ...
                      <p className="text-sm text-gray-500">
                        {t("dashboard.mostMentionedByFrequency")}
                      </p>
                    </CardHeader>
                      ...
                      {!hasReviews ? (
                        <div className="text-center py-8 text-gray-500">
                          ...
                          <p className="text-sm font-medium">
                            {t("dashboard.noReviewsAvailable")}
                          </p>
                          <p className="text-xs mt-1">
                            {t("dashboard.importReviewsToSeeProblems")}
                          </p>
                        </div>
                      ) : topIssues.length > 0 ? (
                        topIssues
                          ...
                          .map((issue: any, index: number) => {
                            ...
                            const mentionCount =
                              issue.count || issue.mentions || 0;
                            const percentage =
                              totalAnalyzed > 0 && mentionCount > 0
                                ? Math.round(
                                    (mentionCount / totalAnalyzed) * 100,
                                  )
                                : 0;
                            ...
                            return (
                              <div
                                key={index}
                                className={`flex items-center justify-between p-3 ${isCritical ? "bg-red-50" : "bg-yellow-50"} rounded-lg`}
                              >
                                <div className="flex items-center gap-2">
                                  <AlertTriangle
                                    className={`w-4 h-4 ${isCritical ? "text-red-500" : "text-yellow-600"}`}
                                  />
                                  <div>
                                    <div className="font-medium">
                                      {translateTheme(
                                        issue.theme ||
                                          issue.issue ||
                                          t("dashboard.unspecifiedProblem"),
                                      )}
                                    </div>
                                    <div className="text-sm text-gray-500">
                                      {percentage > 0
                                        ...
                                        : t("dashboard.identifiedByAI")}
                                    </div>
                                  </div>
                                </div>
                                <Badge
                                  variant={
                                    isCritical ? "destructive" : "default"
                                  }
                                  className={
                                    !isCritical
                        ...
                        <div className="text-center py-4 text-gray-500">
                          <p className="text-sm">
                            {t("dashboard.noProblemsIdentified")}
                          </p>
                          <p className="text-xs mt-1">
                            {t("dashboard.analyzeEstablishmentToSeeProblems")}
                          </p>
                        </div>
                      )}
                    </CardContent>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <p className="text-sm text-gray-500">
                        {t("dashboard.mostMentionedStrengths")}
                      </p>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      {!hasReviews ? (
                        <div className="text-center py-8 text-gray-500">
                          ...
                          <p className="text-xs mt-1">
                            {t("dashboard.importReviewsToSeeStrengths")}
                          </p>
                        </div>
                      ) : topStrengths.length > 0 ? (
                        topStrengths
                          ...
                          .map((strength: any, index: number) => {
                            const mentionCount =
                              strength.count || strength.mentions || 0;
                            const percentage =
                              totalAnalyzed > 0 && mentionCount > 0
                                ? Math.round(
                                    (mentionCount / totalAnalyzed) * 100,
                                  ...
                                  <div>
                                    <div className="font-medium">
                                      {translateTheme(
                                        strength.theme ||
                                          strength.strength ||
                                          t("dashboard.unspecifiedStrength"),
                                      )}
                                    </div>
                                    <div className="text-sm text-gray-500">
                                      {percentage > 0
                        ...
                        <div className="text-center py-4 text-gray-500">
                          <p className="text-sm">
                            {t("dashboard.noStrengthsIdentified")}
                          </p>
                          <p className="text-xs mt-1">
                            {t("dashboard.analyzeEstablishmentToSeeStrengths")}
                          </p>
                        </div>
                      )}
                    </CardContent>
                ...
                {showParetoPoints && (
                  <Card className="mb-8">
                    <CardHeader>
                      ...
                      <p className="text-sm text-gray-600">
                        {t("dashboard.paretoStrengthsDescription")}
                      </p>
                    </CardHeader>
                    <CardContent>
                      <div className="h-80">
                        {(() => {
                          ...
                          const getStrengthColor = (name: string): string => {
                            ...
                            if (
                              lowerName.includes("qualité") ||
                              lowerName.includes("goût") ||
                              lowerName.includes("gout") ||
                            ...
                            if (
                              ...
                              lowerName.includes("cadre") ||
                              lowerName.includes("décor")
                            ) {
                              return "hsl(221, 83%, 53%)"; // Bleu
                            }
                            if (
                              lowerName.includes("service") ||
                              lowerName.includes("accueil") ||
                                ...
                                <Tooltip
                                  ...
                                  formatter={(value, name) => {
                                    if (name === "Cumulative")
                                      return [
                                        `${value}%`,
                                        t("dashboard.cumulativePercent"),
                                      ];
                                    return [
                                      value,
                                      t("dashboard.positiveMentions"),
                                    ];
                                  }}
                                />
                                <Bar
                                  yAxisId="left"
                                  dataKey="count"
                                  name={t("dashboard.positiveMentions")}
                                >
                                  {paretoPointsData.map(
                                    (entry: any, index: number) => (
                                      <Cell
                                        key={`pareto-strength-${index}`}
                                        fill={getStrengthColor(entry.name)}
                                      />
                                    ),
                                  )}
                                </Bar>
                      ...
                      <p className="text-sm text-gray-500 mt-1 mb-0 leading-tight">
                        {t("dashboard.barRepresentsStrengths")}
                      </p>

                      {/* Camembert + Barres - Répartition des points forts */}
                      <div className="mt-2">
                        <h4 className="text-sm font-medium text-gray-700 mb-3 mt-0">
                          {t("dashboard.strengthsDistribution")}
                        </h4>
                        {(() => {
                          const strengthPieData = paretoPointsData.map(
                            (item: any) => ({
                              name: item.name,
                              value: item.count,
                            }),
                          );
                          const strengthTotal = strengthPieData.reduce(
                                      ...
                                      formatter={(
                                        ...
                                        const pct =
                                          strengthTotal > 0
                                            ...
                                            : 0;
                                        return [
                                          `${value} ${t("dashboard.mentions")} (${pct}%)`,
                                          name,
                                        ];
                                      }}
                                    />
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-4 mt-4 text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-muted-foreground">
                              {t("charts.strengths.tasteQuality")}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <div
                            ...
                            ></div>
                            <span className="text-muted-foreground">
                              {t("charts.strengths.niceAmbiance")}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <div
                            ...
                            ></div>
                            <span className="text-muted-foreground">
                              {t("charts.strengths.friendlyStaff")}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <div
                            ...
                            ></div>
                            <span className="text-muted-foreground">
                              {t("charts.strengths.valueForMoney")}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                {showParetoChart && (
                  <Card className="mb-8">
                    <CardHeader>
                      ...
                      <p className="text-sm text-gray-600">
                        {t("dashboard.paretoProblemsDescription")}
                      </p>
                    </CardHeader>
                    <CardContent>
                      <div className="h-80">
                        {(() => {
                          ...
                          const getCategoryColor = (name: string): string => {
                            ...
                            if (
                              lowerName.includes("service") ||
                              lowerName.includes("attente") ||
                              lowerName.includes("lent")
                            ) {
                            ...
                            if (
                              lowerName.includes("qualité") ||
                              lowerName.includes("plat") ||
                              lowerName.includes("qualite")
                            ) {
                              return "hsl(25, 95%, 53%)"; // Orange
                            }
                            if (
                              lowerName.includes("prix") ||
                              lowerName.includes("cher")
                            ) {
                              return "hsl(45, 93%, 47%)"; // Jaune
                            }
                      ...
                      <p className="text-sm text-gray-500 mt-1 mb-0 leading-tight">
                        {t("dashboard.barRepresentsProblems")}
                      </p>

                      {/* Camembert + Barres - Répartition des problèmes */}
                      <div className="mt-2">
                        <h4 className="text-sm font-medium text-gray-700 mb-3 mt-0">
                          {t("dashboard.problemsDistribution")}
                        </h4>
                        {(() => {
                                      ...
                                      formatter={(
                                        ...
                                        const pct =
                                          total > 0
                                            ...
                                            : 0;
                                        return [
                                          `${value} ${t("dashboard.occurrences")} (${pct}%)`,
                                          name,
                                        ];
                                      }}
                                    />
                            ...
                            <div className="p-4 bg-red-50 rounded-lg border-l-4 border-red-500">
                              ...
                              <p className="text-sm text-gray-700">
                                {topIssues.length > 0
                                  ...
                                  : t(
                                      "dashboard.analyzeReviewsToIdentifyFrictionPoints",
                                    )}
                              </p>
                            </div>
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <Badge className="bg-green-600 text-white border-green-600">
                            {checklistActions.filter((a) => a.completed).length}
                            /{checklistActions.length || 5}
                          </Badge>
                          ...
                          checklistActions.map((action) => (
                            <div
                              ...
                              className={`flex items-start gap-3 p-3 rounded-lg border border-gray-200 border-l-4 transition-all duration-300 ${
                                ...
                                if (
                                  text.includes(
                                    "teste le temps d'attente réel aux heures de pointe",
                                  )
                                )
                                  return "border-l-red-500 bg-red-100 hover:bg-red-200";
                                if (
                                  text.includes(
                                    "briefe l'équipe sur la rapidité de service",
                                  )
                                )
                                  return "border-l-red-500 bg-red-100 hover:bg-red-200";
                                if (
                                  text.includes(
                                    "observe le service ce soir pour identifier les points de blocage",
                                  )
                                )
                                  return "border-l-red-500 bg-red-100 hover:bg-red-200";
                                if (text.includes("réponds à 10 avis clients"))
                                  return "border-l-blue-500 bg-blue-100 hover:bg-blue-200";
                                if (
                                  text.includes(
                                    "informe ton équipe du problème prioritaire",
                                  )
                                )
                                  return "border-l-blue-500 bg-blue-100 hover:bg-blue-200";
                                return "border-l-blue-500 bg-blue-100 hover:bg-blue-200";
                            ...
                            >
                              <span
                                className={`text-sm flex-1 ${action.completed ? "line-through text-gray-400" : "text-gray-900"}`}
                              >
                                {action.text}
                              </span>
                              <button
                                onClick={() =>
                                  handleChecklistActionComplete(action.id)
                                }
                  ...
                  <Card className="mb-8">
                    ...
                    <CardContent>
                      <div className="space-y-5">
                        ...
                        {(() => {
                          const consultantAdvice = generateConsultantAdvice(
                            insight,
                            allReviewsForChart,
                            validatedReviews,
                          );
                          ...
                          if (consultantAdvice.length === 0) {
                            return (
                              <div className="text-center py-8 text-gray-500">
                                ...
                                <p className="text-xs mt-1">
                                  {t(
                                    "dashboard.analyzeEstablishmentToGetAdvice",
                                  ) ||
                                    "Analysez votre établissement pour obtenir des conseils personnalisés"}
                                </p>
                              </div>
                            );
                          }
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        ...
                        {(() => {
                          ...
                          const authorName =
                            review.author || t("dashboard.anonymous");
                          const reviewText =
                            extractOriginalText(review.text) ||
                            t("dashboard.noComment");
                          const reviewId =
                            review.id || `review-${currentReviewIndex}`;
                          ...
                          const generateAiResponse = async (
                            ...
                            try {
                              ...
                              if (sessionError) {
                                console.error(
                                  "[generateAiResponse] Erreur session:",
                                  sessionError,
                                );
                              ...
                              if (!session?.access_token) {
                                console.error(
                                  "[generateAiResponse] Pas de token dans la session",
                                );
                                toast.error(t("auth.sessionExpired"), {
                                  description: t("auth.pleaseReconnect"),
                                });
                              ...
                              console.log(
                                "[generateAiResponse] Appel à generate-review-response avec token:",
                                {
                                ...
                                `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-review-response`,
                                {
                                  ...
                                  body: JSON.stringify({
                                    review: {
                                      ...
                                      language:
                                        review.language || review.language_code,
                                    },
                                    establishment: {
                                      name:
                                        selectedEtab?.name ||
                              ...
                              if (!response.ok) {
                                if (response.status === 429) {
                                  ...
                                  toast.error(t("common.error"), {
                                    description:
                                      data.error ||
                                      t("dashboard.cannotGenerateResponse"),
                                  });
                                }
                                return;
                              }
                              ...
                              if (data.response) {
                                setAiGeneratedResponses((prev) => ({
                                  ...prev,
                                  [reviewId]: data.response,
                                }));
                                ...
                                toast.success(
                                  ...
                                  {
                                    description: t(
                                      "dashboard.canModifyBeforeValidate",
                                    ),
                                  },
                                );
                              }
                          ...
                          const defaultResponse = isPositive
                            ? t("dashboard.defaultPositiveResponse", {
                                name: authorName.split(" ")[0],
                              })
                            : t("dashboard.defaultNegativeResponse", {
                                name: authorName.split(" ")[0],
                              });

                          const currentResponse =
                            editedResponses[reviewId] ||
                            aiGeneratedResponses[reviewId] ||
                            defaultResponse;
                                        ...
                                        onClick={async () => {
                                          try {
                                            if (
                                              ...
                                              toast.error(t("common.error"), {
                                                description: t(
                                                  "dashboard.userOrEstablishmentNotDefined",
                                                ),
                                                duration: 4000,
                                              });
                                              return;
                                            ...
                                            if (responsesError) {
                                              console.error(
                                                "Erreur lors du rechargement des réponses:",
                                                responsesError,
                                              );
                                            }
                                            ...
                                            if (responsesData) {
                                              ...
                                              newPending =
                                                allReviewsForChart.filter(
                                                  (review) => {
                                                    const hasValidatedResponse =
                                                      validatedSet.has(
                                                        review.id,
                                                      );
                                                    const hasOwnerReply = !!(
                                                      review.owner_reply_text &&
                                                      review.owner_reply_text.trim()
                                                    );
                                                    const hasRespondedAt =
                                                      !!review.responded_at;
                                                    return (
                                            ...
                                            toast.success(
                                              t(
                                                "dashboard.responseValidatedAndSaved",
                                              ),
                                              {
                                                description: t(
                                                  "dashboard.responseSavedSuccess",
                                                ),
                                                duration: 3000,
                                              },
                                            );
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    toast.success(t("common.copied"), {
                                      description: t(
                                        "dashboard.responseCopiedToClipboard",
                                      ),
                                    });
                                  }}
                          ...
                          <p className="text-2xl font-bold">
                            {(() => {
                              ...
                              if (statusFilter === "pending") {
                                const pendingCount = allReviewsForChart.filter(
                                  (review) => {
                                    const hasValidatedResponse =
                                      validatedReviews.has(review.id);
                                    const hasOwnerReply = !!(
                                      review.owner_reply_text &&
                                      review.owner_reply_text.trim()
                                    );
                  ...
                  <div className="space-y-6">
                    ...
                    {(() => {
                      const isReviewReplied = (review: any) => {
                        const hasValidatedResponse = validatedReviews.has(
                          review.id,
                        );
                        const hasOwnerReply = !!(
                          review.owner_reply_text &&
                      ...
                      const filteredReviews = allReviewsForChart.filter(
                        (review) => {
                          if (statusFilter === "pending")
                            return !isReviewReplied(review);
                            ...
                            <>
                              ...
                              {openCard === "priorite" ? (
                                <>
                                  ...
                                  {(() => {
                                    const watchReviews = filteredReviews.filter(
                                      (r) => r.rating === 3,
                                    );
                      ...
                      >
                        ...
                        <input
                          ...
                          min={Number(
                            Math.min(
                              5,
                              Math.max(1, currentAvgRatingForTarget),
                            ).toFixed(1),
                          )}
                          max={5}
                          step={0.1}
                          value={targetRating}
                          onChange={(e) =>
                            setTargetRating(parseFloat(e.target.value))
                          }
                          ...
                          <div>
                            ...
                            <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${displayAvgRating != null ? (displayAvgRating / 5) * 100 : 0}%`,
                                }}
                              />
                            </div>
                          </div>
                          ...
                          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                            <p className="text-sm text-gray-700">
                              ...
                              {displayAvgRating != null && targetRating > 0
                                ? Math.min(
                                    100,
                                    (displayAvgRating / targetRating) * 100,
                                  ).toFixed(1)
                                : "—"}
                              %
                            </p>
                          ...
                          >
                            ...
                            <div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-green-500 rounded-full transition-all"
                                style={{
                                  width: `${Math.min(100, Math.max(0, l.pct))}%`,
                                }}
                              />
                            </div>
