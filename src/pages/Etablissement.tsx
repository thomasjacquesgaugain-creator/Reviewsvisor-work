...
const Etablissement = () => {
  ...
  const [villeBBox, setVilleBBox] = useState<{
    s: number;
    n: number;
    w: number;
    e: number;
  ...
  const rechercherEtablissementsOpenStreetMap = async (nom: string, villeContext: string) => {
    try {
      setRechercheEnCours(true);
      const queries = [`${nom} ${villeContext}`, `${nom}, ${villeContext}`, `"${nom}" ${villeContext}`];
      let suggestions: any[] = [];
  ...
  const rechercherVillesAutomatique = async (nomVille: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const villeTypes = new Set(["city", "town", "village", "municipality", "administrative"]);
        const suggestions = data.filter((item: any) => item.class === "place" && villeTypes.has(item.type) || item.class === "boundary" && item.type === "administrative" && (item.address?.city || item.address?.town || item.address?.village)).map((item: any) => ({
          id: item.place_id,
          ...
          {modeActuel === 'recuperation' && <Card className="mb-8">
              <CardContent className="p-8">
                <div className="grid md:grid-cols-3 gap-6 mb-6">
                  <div className="space-y-2 px-[380px]">
                    <label className="text-sm font-medium text-gray-700">
                      Établissement <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <Input placeholder="Nom de votre établissement..." value={etablissement} onChange={e => {
                    setEtablissement(e.target.value);
                    rechercherEtablissementsAutomatique(e.target.value, ville);
                  }} className="w-full" />
                      {rechercheEtablissementsEnCours && <div className="absolute right-3 top-3">
                          <div className="animate-spin rounded-full h-4 w-4 border-2 border-blue-600 border-t-transparent"></div>
                        </div>}
                      
                      {/* Suggestions dropdown */}
                      {suggestionsEtablissements.length > 0 && <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-xl max-h-60 overflow-y-auto">
                          {suggestionsEtablissements.map(suggestion => <div key={suggestion.id} onClick={() => selectionnerEtablissement(suggestion)} className="px-4 py-3 hover:bg-blue-50 cursor-pointer border-b border-gray-100 last:border-b-0 transition-colors">
                ...
                <div className="flex gap-4">
                  <Button className="w-full" onClick={rechercherEtablissement} disabled={rechercheEnCours}>
                    {rechercheEnCours ? <>
