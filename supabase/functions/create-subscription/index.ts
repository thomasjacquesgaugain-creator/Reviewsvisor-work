...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Establishment count", {
      total: totalEstablishments,
      additional: additionalEstablishments,
    });
    ...
    const origin =
      ...
      "https://reviewsvisor.fr";

    // ======= ADMIN BYPASS =======
    if (user.email.toLowerCase() === ADMIN_EMAIL.toLowerCase()) {
      logStep("Admin bypass detected", {
        email: user.email,
        priceId: mainPriceId,
      });
      ...
      if (!productKey) {
        logStep("ERROR: Unknown priceId for admin bypass", {
          priceId: mainPriceId,
        });
      ...
      const planKey =
        productKey === "pro_1499_12m" ? "pro_1499_12m" : "pro_2499_monthly";
      ...
      if (existingEntitlement) {
        ...
        if (updateError) {
          throw new Error(
            `Failed to update entitlements: ${updateError.message}`,
          );
        }
      } else {
        const insertData = {
        ...
        if (insertError) {
          throw new Error(
            `Failed to insert entitlements: ${insertError.message}`,
          );
        }
      }

      logStep("Admin subscription activated", {
        userId: user.id,
        productKey,
    ...
    if (additionalEstablishments > 0) {
      ...
      logStep("Adding additional establishments to checkout", {
        quantity: additionalEstablishments,
      });
    }
