...
> = {
  ...
  addon_multi_etablissements_499: {
    type: "addon",
    addonField: "addon_multi_etablissements",
  },
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (!CREATOR_BYPASS_EMAILS.includes(user.email.toLowerCase())) {
      ...
      throw new Error(
        "Unauthorized: Creator bypass not available for this account",
      );
    }
    ...
    if (existingEntitlement) {
      ...
      if (config.type === "pro") {
        ...
        updateData.addon_multi_etablissements_qty =
          (existingEntitlement.addon_multi_etablissements_qty || 0) + 1;
